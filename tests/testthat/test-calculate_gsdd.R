...
test_that("vector must be longer than rollmean_units", {
  set.seed(13)
  day <- 1:365
  x <- -15 * cos((2*pi / 365) * (day-10)) + rnorm(365, mean = 10, sd = .5)
  expect_error(calculate_gsdd(x, rollmean_units = 369, start_temp = 5, end_temp = 4, n_consecutive = 5))
})
...
test_that("vector must be longer than n_consecutive", {
  ...
  day <- 1:365
  x <- -15 * cos((2*pi / 365) * (day-10)) + rnorm(365, mean = 10, sd = .5)
  expect_error(calculate_gsdd(x, rollmean_units = 7, start_temp = 5, end_temp = 4, n_consecutive = 369))
})
...
test_that("vector must not contain NA values", {
  ...
  x[random_indices] <- NA
  expect_error(calculate_gsdd(x, rollmean_units = 7, start_temp = 5, end_temp = 4, n_consecutive = 5))
})
...
test_that("start temp must be greater than or equal to end temp", {
  ...
  day <- 1:365
  x <- -15 * cos((2*pi / 365) * (day-10)) + rnorm(365, mean = 10, sd = .5)
  expect_error(calculate_gsdd(x, rollmean_units = 7, start_temp = 5, end_temp = 40, n_consecutive = 5))
})
...
test_that("if max temp in vector is lower than start_temp the function will error", {
  ...
  day <- 1:365
  x <- -15 * cos((2*pi / 365) * (day-10)) + rnorm(365, mean = 10, sd = .5)
  expect_error(calculate_gsdd(x = x, rollmean_units = 7, start_temp = 50, end_temp = 4, n_consecutive = 5))
})
...
test_that("if end_temp is not reached, gsdd calculated to end of vector and warning is shown", {
  ...
  day <- 1:365
  x <- -15 * cos((2*pi / 365) * (day-10)) + rnorm(365, mean = 10, sd = .5)
  expect_warning(calculate_gsdd(x, rollmean_units = 7, start_temp = 5, end_temp = -40, n_consecutive = 5))
})
