test_that("errors when date_time column is missing", {
  data <- data.frame(
...
test_that("errors when date_time column type is not a date time", {
  data <- data.frame(
...
test_that("errors when value column is missing", {
  data <- data.frame(
...
test_that("errors when value column type is not numeric", {
  data <- data.frame(
    date_time = as.POSIXct(c("2021-05-07 08:00:00")),
    value = c("cold")
  )
...
test_that("status_id column is output as a factor", {
  output_1 <- classify_time_series_data(
    data.frame(
      date_time = as.POSIXct(c("2021-05-07 08:00:00")),
      value = c(0)
    )
  ...
  expect_error(
    classify_time_series_data(
      data.frame(
        ...
        value = c(0, 0.123, 0.345)
      )
    ),
    regexp = "`data\\$date_time` must be unique."
  )
})
...
test_that("errors when missing a date time", {
  expect_error(
    ...
    regexp = "`data\\$date_time` must not have any missing values."
  )
})
